...
func New[T comparable, S Subscriber[T]](conf *Config) *EventEmitter[T, S] {
	...
	for i := int64(0); i < conf.BucketNum; i++ {
		buckets = append(buckets, &bucket[T, S]{
			...
			Topics: treemap.New[*topicField[T, S]](conf.Separator),
		})
	}
...
func (c *EventEmitter[T, S]) Subscribe(suber S, topic string, f func(subscriber S, msg any)) {
...
type bucket[T comparable, S Subscriber[T]] struct {
	sync.Mutex
	Size   int64
	Topics *treemap.TreeMap[*topicField[T, S]]
}

// 新增订阅
func (c *bucket[T, S]) subscribe(suber S, topic string, f eventCallback[T, S]) {
	c.Lock()
	defer c.Unlock()

	subId := suber.GetSubscriberID()
	ele := topicElement[T, S]{suber: suber, cb: f}

	if t, ok := c.Topics.Get(topic); ok {
		t.subers[subId] = ele
	} else {
		t = &topicField[T, S]{subers: make(map[T]topicElement[T, S], c.Size)}
		t.subers[subId] = ele
...
func (c *bucket[T, S]) publish(topic string, msg any) {
	...
	c.Topics.Match(topic, func(_ string, t *topicField[T, S]) {
